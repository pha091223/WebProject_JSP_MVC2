...
<body>
	<c:choose>
		<c:when test="${writecheck=='true' }">
			<script>
				alert("글이 등록되었습니다.");
			</script>
		</c:when>
		<c:when test="${writecheck=='false' }">
			<script>
				alert("글이 등록되지 않았습니다.");
			</script>
		</c:when>
    ...
    <div class="container clearfix">
      <div class="content col-lg-12 col-md-12 col-sm-12 clearfix">
        ...
        <div id="bbpress-forums">
          <ul class="bbp-forums">
            <li class="bbp-header">
              <ul class="forum-titles">
                ...
                <li class="bbp-forum-topic-count">작성자</li>
                <li class="bbp-forum-topic-count">작성날짜</li>
                <li class="bbp-forum-topic-number">조회수</li>
              </ul>
            </li>
            <!-- .bbp-header -->
	
			<c:forEach items="${boardList }" var="fbList" begin="${pagemaker.nowPageStart }" varStatus="bs">
	            <li class="bbp-body">
	              <ul class="forum type-forum status-publish hentry loop-item-0 odd bbp-forum-status-open bbp-forum-visibility-publish">
	                <li class="bbp-forum-topic-count">${fbList.no }</li>
	                <li class="bbp-forum-info">
	                	<c:forEach begin="1" end="${fbList.lvl }">
							<span>&nbsp;&nbsp;</span>
						</c:forEach>
						<c:if test="${fbList.lvl > 0 }">
							<span>[re:]</span>
					  	</c:if>
	                  <a class="bbp-forum-title" 
	                  		href="Servlet?command=noticebbs_view&no=${fbList.no}&page=${pageMaker.prev }" title="General">
	                 	 	${fbList.title }</a>
	                </li>
	                <li class="bbp-forum-topic-count">${fbList.name }</li>
	                <li class="bbp-forum-topic-count">${fbList.day }</li>
	                <li class="bbp-forum-topic-number">${fbList.readcount }</li>
	              </ul>
	              <!-- end bbp forums -->
	            </li>
            </c:forEach>
		  ...
		  <c:if test="${sessionScope.nowuser_Id==name }">
	          <div class=" bbsbutton">
	            <a class=" dmbutton" href="javascript:bbs_authority_check('noticebbs', '${sessionScope.nowuser }');">글쓰기</a>
	          </div>
		  </c:if>
        ...
        <div class=" text-center">
          <ul class="pagination">
          	<c:if test="${pageMaker.prev>0 }">
				<li><a href="Servlet?command=noticebbs_list&page=${pageMaker.prev}">«</a></li>
			</c:if>
            <c:forEach begin="${pageMaker.start }" end="${pageMaker.end}" var="idx">
	            <li><a href="Servlet?command=noticebbs_list&page=${idx}">${idx }</a></li>
            </c:forEach>
            <c:if test="${pageMaker.next>0 }">
				<li><a href="Servlet?command=noticebbs_list&page=${pageMaker.next}">»</a></li>
			</c:if>
          </ul>
       </div>
      <!-- end content -->
